...
- OIDC による GitHub → GCP 認証設定（推奨）
  ...
  - 対象サービスアカウントへのロール付与（`roles/run.admin`,
...
- フロー: checkout → setup-node → build → auth(OIDC) → setup-gcloud →
  deploy-cloudrun → URL 出力
...
- 認可エラー:
  SA に必要ロールが付与されているか確認。OIDC の audience と subject の一致を見直し。
- リージョン不一致: `CLOUD_RUN_REGION`
  が実環境のリージョンと一致しているか確認。
- ビルド失敗: `npm ci`/`npm run build`
  をローカルで再現。キャッシュクリア（`npm cache clean --force`）。
- URL 取得失敗: `gcloud run services describe ...`
  で直接確認。サービス名/リージョンを再チェック。

## 今後の強化

- アクションの SHA ピンニング（供給網対策）
- デプロイ前に 06-ci-build の成果物を再利用（アーティファクト／Cloud
  Build キャッシュ）
